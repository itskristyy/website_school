...
<head>
    ...
    <title>Direktori Siswa - Skanifo</title>
...
<body data-bs-spy="scroll" data-bs-target="#mainNav" data-bs-offset="90" tabindex="0">
    ...
    <main class="container" style="margin-top: 120px; margin-bottom: 80px; min-height: 60vh;">
        <div class="text-center mb-5">
            <h1 class="font-headline fw-bold" style="color: var(--skn-primary);">Direktori Siswa</h1>
            <div class="mx-auto mt-3" style="width: 80px; height: 4px; background-color: var(--skn-amber);"></div>
            <p class="text-muted mt-3 font-mono">Daftar siswa aktif SMK Skanifo.</p>
        </div>

        <div class="row row-cols-1 row-cols-md-2 g-4">
            <?php
            include '../../function/connect.php';
            $data = mysqli_query($koneksi, "SELECT * FROM tb_siswa ORDER BY id_siswa DESC");
            if(mysqli_num_rows($data) > 0){
                    ...
                    <div class="col">
                        <div class="card border-0 shadow-sm rounded-0 h-100" style="border-left: 6px solid var(--skn-primary) !important; background-color: var(--skn-bg);">
                            <div class="card-body d-flex align-items-center gap-4 p-4">
                                ...
                                <!-- Info Siswa -->
                                <div>
                                    <h4 class="font-headline fw-bold mb-1" style="color: var(--skn-primary);"><?php echo htmlspecialchars($row['nama_siswa']); ?></h4>
                                    <div class="font-mono text-muted mb-2 small">NIPD: <?php echo htmlspecialchars($row['nipd']); ?></div>
                                    <span class="badge rounded-0 px-3 py-2 text-uppercase font-mono" style="background-color: var(--skn-amber); color: var(--skn-primary);">
                                        <?php echo htmlspecialchars($row['siswa_jurusan']); ?>
                                    </span>
                                </div>
                            </div>
                        </div>
                ...
                echo "<div class='col-12 text-center text-muted'>Belum ada data siswa.</div>";
            }
            ?>
        </div>
    </main>
